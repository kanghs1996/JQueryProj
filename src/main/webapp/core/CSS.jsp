...
$(function(){
	...
	$('#get').on('click',function(){
		...
		console.log('배경색:%s,%s',back,div.get(0).style.backgroundColor);//배경색:rgb(255, 0, 0),red
	...
	$('#set').click(function(){
		//메소드 체인으로]
		//$('div').css('width','200px').css('height','200px').css('background-color','green');
		//JSON타입으로
		$('div').css({'width':'200px','height':'200px','backgroundColor':'rgb(0,255,0)'});
	});
	...
	$('a').click(function(){
		...
		switch($(this).html()){
			case '크게':
				pTag.css('fontSize',fontSize+5+'px');
				break;
			case '작게':
				pTag.css('fontSize',fontSize-5+'px');
				break;
			case '진하게':
				pTag.css('fontWeight','bold');
				break;
			case '보통':
				pTag.css('fontWeight','normal');
				break;
			case '빨강':
				pTag.css('color','red');
				break;
