...
$(function(){
	...
	*/	
	...
	console.log(':button선택자:',$(':button').length);//3,<button>도 포함된
	/*
   ...
   $('button').click(function(){
	   switch($(this).html()){
	   		case '값 읽기':
	   			...
	   			$(":button[type=button]")
	   			*/
	   			//console.log($(':button').val());//무조건 첫번째 요소
	   			$(':button').each(function(i){
	   				console.log('%d번째 버튼의 type속성:%s',i,$(this).prop('type'));
	   				if($(this).is('[type=button]')){
	   					console.log('%d번째 버튼의 value속성:%s',i,$(this).val());
	   				}
	   				else{
	   				//<button>태그일때는 val()함수로 읽지 못한다 즉 value속성이 없다
	   					//console.log('%d번째 버튼의 value속성:%s',i,$(this).val());//빈문자열
				 ...
				 */
				 ...
				 //4-1]$(":checkbox").val():체크에 상관없이 첫번째 값만 읽어옴.	
	   			...
	   			console.log('3. :checkbox와 if로 체크된것만 판단 그리고 each()함수');
				...
				*/
				...
				//console.log("$(':submit').val():",$(':submit').val());//<button>태그 포함
				...
				:select 폼 선택자 없다
				*/
				//console.log($(':select'));//Syntax error
