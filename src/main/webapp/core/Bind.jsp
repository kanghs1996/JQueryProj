...
$(function(){
	...
	//$('div:last-child').css('color','red');//선택된 2개의 div에 다 적용 됨
	//html():Get the HTML contents of the first element in the set of matched elements.
	//console.log($('div:last-child').html());//네이트(선택된 객체중 첫번째 객체에만 적용됨)
	//선택된 각각의 hmtl()함수를 적용해야한다:.each()함수
	$('div:last-child').each(function(){
		console.log($(this).html());		
	});
	//메모리가 생성된 순서대로 인덱스가 부여된다.아래코드 실행시 네이버가 2(마지막 div가 2가 아니다)
	...
	$('button').bind('click',function(){
		if($(this).html().indexOf('버튼') != -1)
			var buttonText=$(this).html();
		else
			var buttonText=$(this).html()+"버튼이 클릭되었어요";
		$(this).html('');
		$(this).html("<strong>"+buttonText+"</strong>");
	...
	//this 와 $(this)의 차이점]
	/*
	this:자바스크립트의 this키워드.해당 태그의 속성값을 얻어올때 사용 예] this.태그속성명
	...
	$(this) 와 this는 이벤트 핸들러에서 이벤트가 발생한 객체를 의미한다
	*/
	$("div:eq(1) div").bind('click',function(){
		console.log("$(this).title:",$(this).title);//undefined
		console.log("$(this).get(0).title:",$(this).get(0).title);//[o]
		console.log("this.title:",this.title);//[o]
		switch(this.id){
